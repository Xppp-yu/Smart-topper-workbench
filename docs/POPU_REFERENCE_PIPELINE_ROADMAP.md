...
- 输入：P5.2-A 通过的候选 + 开发受试者固定子集（冻结 `["1","2","3","4","5","6"]`，看结果前固定）+ 冻结 P2 质量 manifest（`primary` = ACCEPT-only cohort，WARN/EXCLUDED 不入 Mini）。
...
- 就绪产物：[协议报告](stage_reports/P5_2_B_POPU_NEURAL_MINI_PROTOCOL_v0.1.md)、[冻结配置](../configs/experiments/popu_neural_mini_v0.1.json)、`runner_type=popu_neural_mini`；Reviewer 首轮复核返回 `REVIEW_NEEDS_FIX`，已按 6 项要求完成修复提交；尚未运行真实 Mini/Full。
