...
| A4 固定姿态 Baseline | A3 特征 + 姿态标签 | 规则 / Logistic / RF / 可选小型 CNN，受试者隔离比较 | predictions、混淆矩阵、逐受试者指标 | 只在 validation 选模型；test 最后一次使用 |
...
A2 Mask / Geometry      已有首版，P3.1 等待比较冻结
A1 区域标签对齐         P3.2 代码已就绪，等待真实审计输出
A3 无标签特征           等待 Mask 规则冻结
A4 姿态 Baseline        尚未开始
A5 区域候选             等待标签配对结论
A6 鲁棒性/密度          尚未开始
...
1. 跑完 P3.1，冻结一个**研究用**接触 Mask 规则；
2. 跑完 P3.2，给 COCO 标注明确判定：可作何种层级的监督、不能作何种真值；
3. 进入 A3/P4a，生成不依赖区域真值的特征表；
4. 同时起草 B0/B1：自研传感器坐标、采样、同步真值和标注协议；
5. 有了 A4/A5 的候选结果后，再定义硬件验证的最低验收门槛，而不是反过来按模型成绩补故事。
...
## 当前最关键的卡点
1. 接触 Mask 还没有冻结
目前使用的简单阈值可能保留人体周围的零散弱信号，导致边界框变大、主轴偏移。
因此还不能把现有 Geometry 当作最终特征输入。P3.1需要比较：
相对阈值；
最大连通域；
阈值加形态学闭运算；
同一记录相邻帧的稳定性；
代表样本的人工叠加图。
这是当前最近、最明确的执行卡点。
2. 身体区域标签与压力帧是否对应，还不知道
本地有1,730份PoPu COCO标注，但目前没有证明：
它对应哪一位受试者；
对应哪个姿态记录；
对应哪个snapshot；
坐标方向、尺寸和压力矩阵是否一致。
所以现在不能拿这些标签直接训练肩、腰、骨盆识别。P3.2即使最后得到“无法逐帧可靠配对”，也是有效结论：PoPu只能用于结构参考，区域监督训练继续 HOLD。
3. “肩、腰、骨盆真值”本身还没完全定义
需要先冻结标注合同，例如：
肩是左右肩关键点的中点，还是肩部区域？
腰是一个纵向坐标，还是一段腰椎区域？
...
4. 独立真值数据尚未正式接入工作台
目前运行链主要是PoPu。TIP、SLP、PressurePose仍属于“已找到的候选数据源”，还没有：
完整下载与校验；
建立版本记录；
编写独立 Adapter；
检查矩阵和人体坐标映射；
...
8. 版本冻结目前也有风险
我刚检查到这个Git仓库的 main 分支目前没有任何commit，所有文件都还是未跟踪状态。这意味着现在虽然有报告和产物，但还没有真正冻结一个可回退、可对比的版本。
另外有少量文档口径需要同步，例如P2阶段报告仍写着 PARTIAL，而状态看板已经把P2.1策略确定后标成 COMPLETE。这不阻塞算法，但会影响团队复盘。


## 下一步最合理的推进顺序
运行P3.1，比较Mask策略并冻结研究用规则。
运行P3.2，审计PoPu COCO标签到底能用到什么程度。
不等待区域真值，先推进P4a无标签特征表。
同时冻结“肩—腰—骨盆”的真值与评价合同。
建立TIP、SLP、PressurePose各自的下载记录、Manifest和Adapter。
先做R1/R2/R3低成本Baseline，再做R4/R5图像模型。
按相同受试者切分和指标公平比较。
再做UNKNOWN、降密度、坏点和噪声测试。
