...
| A4 固定姿态 Baseline | A3 特征 + 姿态标签 | 规则 / Logistic / RF / 可选小型 CNN，受试者隔离比较 | predictions、混淆矩阵、逐受试者指标 | 只在 validation 选模型；test 最后一次使用（P5 v0.1 曾对每个候选各测一次，P5.1 收紧为仅最终候选） |
...
A4 姿态 Baseline        P5 首轮基线已完成（logreg=当前首轮领先候选，未冻结）；P5.1 横向复核待做
A5 区域候选             等待标签配对结论
...
4. P5 首轮受试者隔离基线已跑通，`logreg` 为当前首轮领先候选；下一步 P5.1 横向比较复核，通过后才冻结候选；
...
P5 首轮 Baseline（已完成，候选未冻结）
    ↓ P5.1 横向比较：严格“仅对最终候选一次 held-out test”口径 + 模块化增强 + 候选复核（完成前不冻结模型/UNKNOWN 阈值）
...
目前运行链主要是PoPu。TIP、SLP、PressurePose已下载到本地数据副本（状态为 `PRESENT_NOT_INTEGRATED`），但还没有：
建立版本记录；
编写独立 Adapter；
检查矩阵和人体坐标映射；
生成统一 Manifest。
...
PoPu 后续推进顺序：P5.1 横向比较（严格单次 test 口径 + 模块化 + 候选复核）→ P6 UNKNOWN/REJECT → P7 软件鲁棒性（降密度、坏点、噪声）→ PoPu 参考验证包。
P5.1 完成前不正式冻结模型或 UNKNOWN 阈值。
区域监督继续 HOLD，不等待区域真值即可继续 P5.1/P6/P7。
同时冻结“肩—腰—骨盆”的真值与评价合同。
TIP、SLP、PressurePose 已下载到本地数据副本，但尚未建立各自的下载记录、Manifest 和 Adapter；它们是后续接力真值候选，不能与 PoPu 逐行拼接。
