...
| S2_B04A | SLP8 PM-only 受控架构扩展 Mini | PROTOCOL_ACCEPTED / IMPLEMENTATION_SMOKE_ACCEPTED / RUNNER_INTEGRATION_NOT_STARTED | [B04A Protocol](stage_reports/S2_B04A_SLP8_PM_ARCHITECTURE_EXPANSION_MINI_PROTOCOL_v0.1.md)；[B04A Implementation+Smoke](stage_reports/S2_B04A_IMPLEMENTATION_SMOKE_v0.1.md)；[B04A 交付说明](deliverables/SLP/B04A_PM_ARCHITECTURE_EXPANSION_MINI_PROTOCOL_v0.1.md)；[B04A Implementation+Smoke 任务合同](tasks/TASK_SLP_B04A_IMPLEMENTATION_SMOKE_v0.1.md)；[B04A Protocol 任务合同](tasks/TASK_SLP_B04A_PROTOCOL_FREEZE_v0.1.md)；[配置](../configs/experiments/slp8_pm_architecture_expansion_mini_v0.1.json)；3 候选：`slp8_small_unet_v0.1`（incumbent,118,121）/`slp8_resunet_lite_v0.1`（120,809，3 residual block 全部 1x1 Conv2d shortcut；shortcut 精确分解 32+544+2112=2,688 = ResUNet-SmallUNet）/`slp8_deeplabv3plus_lite_v0.1`（53,449，Option A plain atrous 6 分支 ASPP）；SegFormer-B0 DEFERRED；阈值 0.355644；seeds=[42,123,2026]；aug=none；参数上限 300,000；`tests/test_b04a_implementation.py` **79 / 79**（含 R02 新增 shortcut 精确分解 + smoke refuse-overwrite / no-write / declarative-policy 测试）+ 协议验证器 30 OKs/0 errors + 协议验证器单元测试 + 链接检查 56 / 56 + 原有 `test_slp8_region_models.py` 38 / 38 + `test_slp8_region_mini.py`（SmallUNet/registry/synthetic）15 / 15 = **173 passed** 核心套件；另有 B04 相关回归 **15 / 15**；CPU no-write Smoke 全候选 forward+backward+checkpoint roundtrip+same-seed 通过；CUDA Smoke 显式 `NOT_RUN`（本机 `torch==2.13.0+cpu` CPU-only build）；smoke 脚本支持 `--output` / `--force` / `--no-write`，默认拒绝覆盖已存在输出；`test_access.kind = "declarative_policy"`（非运行时计数）；**现有 B04 runner 拒绝 B04A config**（实测 `ConfigValidationError`：`B04_CANDIDATE_NAMES` 不含 B04A 候选名 + `task_id` 不匹配），因此 runner integration **未完成**；`git diff --check` 干净；TEST = 0；临时未跟踪探针已清理；阶段名 = `IMPLEMENTATION_SMOKE_ACCEPTED`（仅接受实现与合成 Smoke；**不得**标记 `RUNNER_INTEGRATION_COMPLETE`/`GPU_MINI_AUTHORIZED`/`MINI_COMPLETE`/`B07_READY`） | 下一允许：另开 `TASK-SLP-B04A-RUNNER-INTEGRATION-SMOKE-v0.1`（扩展 `B04_CANDIDATE_NAMES` / task_id / 配置 schema，让 B04 runner 接受 B04A config 并跑 runner-integration smoke；仍不允许 GPU Mini）；之后才能 B04A-MINI-RUN（Owner 单独授权 + 真实 GPU）；B07 仍 BLOCKED_BY_B04A |
