...
| P7/R7 | 降密度、噪声、坏点、坏行坏列 | COMPLETE — SOFTWARE_PERTURBATION_ONLY | [P7 Full 结果与复核](stage_reports/P7_POPU_SOFTWARE_ROBUSTNESS_FULL_RESULTS_v0.1.md)；14 conditions × 5 seeds × 15 folds，归档与 Reviewer anchors 已复核 | 只允许软件扰动结论；硬件故障、产品、整夜和控制安全需独立 EXP-ID |
...
| S2_B04A | SLP8 PM-only 受控架构扩展 Mini | ROUTE_DOCUMENTATION_ACCEPTED / PROTOCOL_NOT_STARTED / IMPLEMENTATION_NOT_STARTED | [B04A 任务合同](tasks/TASK_SLP_B04A_PM_ARCHITECTURE_EXPANSION_MINI_v0.1.md)；路线文档 Reviewer 已接受；候选假设、增强 Gate、TEST 禁令和最大晋级数已记录 | 下一允许任务是独立冻结 B04A Protocol；协议经 Reviewer 接受后才能另开实现/Smoke，GPU Mini 仍需 Owner 单独授权 |
