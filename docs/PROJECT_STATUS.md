...
| S2_B08 | SLP8 PM-only Full Runner 与 one-fold preflight runner | R01_FAILED / R02_FAILED_AT_FIRST_LOSS / R03_RUNNER_FROZEN / GPU_REAUTH_PENDING | R02 在 strict deterministic + Torch 2.13 CUDA NLLLoss2d 首 loss forward 失败，zero optimizer steps；唯一 FAILED 生效但 error 被空 OOF stack 二次异常覆盖。Round 8 改用数学等价 deterministic cross entropy，FAILED 不写 OOF、DONE 强制精确 OOF coverage；runner=`02fb364902736a64ee8708440f0dd0bdddf860bc`；B08 80 passed、links 6 passed、CPU 数值/梯度与本机 strict CUDA model probe 通过；TEST=0 | 推送 baseline/制作 R03 bundle；AutoDL 先做随机张量 primitive probe；R03 需 Owner 重新授权，Full/TEST 未授权 |
