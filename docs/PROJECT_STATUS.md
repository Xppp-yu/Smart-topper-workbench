...
最后更新：2026-09-04（B11F final-fit R02 三 checkpoint 审计 ACCEPT；B09T 一次性评价协议已冻结待独立审查，TEST 未授权）
...
| S2_B09T | SLP8 一次性最终 TEST 评价 | PROTOCOL_ITERATE_P1_FIXED / READY_FOR_REREVIEW / TEST_DENIED / EXECUTION_NOT_AUTHORIZED | [协议任务](tasks/TASK_SLP_B09T_FINAL_TEST_PROTOCOL_FREEZE_v0.1.md)、[协议报告](stage_reports/S2_B09T_FINAL_TEST_PROTOCOL_FREEZE_v0.1.md)、[冻结配置](../configs/experiments/slp8_pm_b09t_final_test_protocol_v0.1.json)；绑定 B11F 三 final SHA；primary=三 seed hard plurality（2/3 majority；全异固定 seed 42 tie-break 并审计）；primary metric=pooled fixed foreground macro IoU；canonical protocol identity 整体锁定；11 subjects / 495 samples 仅为结构合同，未读取 | 重新独立审查两项 P1；通过后实现 no-TEST evaluator/runner；正式 TEST 仍需一次性 Owner 精确授权 |
