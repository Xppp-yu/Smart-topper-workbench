...
| S2_B04A | SLP8 PM-only 受控架构扩展 Mini | DOCUMENTATION_REVIEW_PENDING / IMPLEMENTATION_NOT_STARTED | [B04A 任务合同](tasks/TASK_SLP_B04A_PM_ARCHITECTURE_EXPANSION_MINI_v0.1.md)；候选假设、增强 Gate、TEST 禁令和最大晋级数已记录 | 先 Reviewer 接受协议，再另开实现/Smoke 任务；GPU Mini 仍需 Owner 单独授权 |
