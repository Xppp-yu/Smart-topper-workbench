...
P5.2-C 已接受 `small_resnet` 为 PoPu 固定睡姿五分类总体研究候选模型族。P6/P6.1 已完成 UNKNOWN/REJECT、原始重点案例复核、温度校准与三模型一致性模拟；P7 真实 checkpoint 扰动推理仍待运行。SLP A09R 已以 `COMPLETE_WITH_LIMITATIONS` 验收：`SLP_8Region_Pressure_VAL_v1.1`（4,590 samples，102 danaLab，8 区）确立为当前项目 SLP8 pressure-only 区域分割参考 GT，8 区 schema + adapter（66 tests）+ 全量 validator（4590/4590，0 failures）和 A06 split 兼容性（3645/450/495，0 overlap）均经 Reviewer 复核；A10–A17 OpenCV/人工复核路线为 HOLD/SUPERSEDED。SLP B01 已以 `DONE_WITH_LIMITATIONS` 验收：pressure-only 8-region 训练/验证/测试数据入口（`slp8_training_tables_v0.1`）包含 3645/450/495 manifests、TRAIN-only normalization（`raw_pmarray_response`，NOT kPa）、TEST 防泄漏合同、数据集卡与顶层 freeze manifest。SLP B02 现已以 `DONE_WITH_LIMITATIONS` 验收：四种 pressure-only 非学习基线完成 TRAIN/VAL CPU 运行，VAL fixed IoU 分别为 `0.000000`、`0.205644`、`0.109308`、`0.109713`；R03 fail-closed 修复后联合验收为 259 passed、2 skipped；未读取 TEST。B03 保持 `READY`，B04 现仅由 B03 阻塞。
