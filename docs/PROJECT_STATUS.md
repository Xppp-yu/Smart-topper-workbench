...
最后更新：2026-09-04（B09T 协议 ACCEPT；no-TEST evaluator/runner 与 synthetic smoke 已实现待独立审查；TEST 未授权）
状态口径：只把已运行且存在可追溯产物的步骤标记为 `COMPLETE`；代码、计划或空目录不等于完成；验收前保持 `READY_FOR_REVIEW`。

## 当前一句话状态

PoPu P5.2-C 已接受 `small_resnet` 为固定睡姿五分类总体研究候选模型族；P6/P6.1 已完成 UNKNOWN/REJECT、错误复核、温度校准与三模型一致性模拟；P7 Full 软件扰动证据已完成并复核，但不构成硬件、产品、整夜睡眠或控制安全验证。SLP B09 R01 已在 `8b3ebda`、RTX 4090、TEST=0 下完成 30/30 units；DeepLabV3+-lite 以 mean pooled IoU `0.494134` 胜过 ResUNet-lite 的 `0.456795`。B11F final-fit R02 已在冻结 runner `a6a5d8e`、RTX 4090、TEST=0 下完成 seeds 42/123/2026（epochs 15/20/12）；唯一根终态为 DONE，三 final checkpoint 的 SHA、identity、reload、有限数值和预算 carrier 已独立审计 `ACCEPT`。B09T 一次性评价协议已冻结为配置并通过本地 fail-closed 验证；当前 Gate 为 `B09T_PROTOCOL_INDEPENDENT_REVIEW / TEST_DENIED / EXECUTION_NOT_AUTHORIZED`。任何 TEST 读取仍需后续另一份一次性 Owner 精确授权。
...
| S2_B11 | SLP8 研究候选合同冻结 | CANDIDATE_CONTRACT_ACCEPTED / FINAL_FIT_COMPLETE / TEST_DENIED | [B11 报告](stage_reports/S2_B11_SLP8_RESEARCH_CANDIDATE_FREEZE_v0.1.md)、[冻结配置](../configs/experiments/slp8_pm_research_candidate_v0.1.json)；DeepLabV3+-lite；全开发集三 seed final-fit epochs 15/20/12；hard majority + 可选 3/3 `UNKNOWN_REGION`；B11F R02 已完成三 checkpoint 审计 | B09T 协议可起草；TEST 继续禁止，执行需另一份一次性 Owner 授权 |
| S2_B11F | SLP8 最终开发集拟合 runner | FINAL_FIT_R02_ACCEPTED / THREE_CHECKPOINTS_AUDITED / TEST_DENIED | [runtime 修复报告](stage_reports/S2_B11F_RUNTIME_CLASS_WEIGHT_FIX_v0.1.md)、[R01 原始终态证据](evidence/b11f_r01_failure/README.md)、[smoke 报告](stage_reports/S2_B11F_PRODUCTION_WIRING_SMOKE_v0.1.md)、[preflight 报告](stage_reports/S2_B11F_AUTODL_NO_TRAINING_PREFLIGHT_PREPARATION_v0.1.md)、[R02 授权包报告](stage_reports/S2_B11F_FINAL_FIT_R02_AUTHORIZATION_PACKAGE_v0.1.md)、[R02 结果与审计](stage_reports/S2_B11F_FINAL_FIT_R02_RESULTS_AND_AUDIT_v0.1.md)；RTX 4090、runner `a6a5d8e`、TEST=0；唯一 DONE；seeds 42/123/2026、epochs 15/20/12；三 final SHA/reload 和六 checkpoint 有限性通过；约 279.819/2700 秒，peak CUDA 约 330.951 MiB；exitcode 文件为无效 post-run capture，作为永久 provenance limitation 保留 | B09T 协议已起草待独立审查；不得读取 TEST，执行需另一份一次性 Owner 精确授权 |
| S2_B09T | SLP8 一次性最终 TEST 评价 | PROTOCOL_ACCEPTED / NO_TEST_EVALUATOR_READY_FOR_REVIEW / TEST_DENIED / EXECUTION_NOT_AUTHORIZED | [协议任务](tasks/TASK_SLP_B09T_FINAL_TEST_PROTOCOL_FREEZE_v0.1.md)、[协议报告](stage_reports/S2_B09T_FINAL_TEST_PROTOCOL_FREEZE_v0.1.md)、[冻结配置](../configs/experiments/slp8_pm_b09t_final_test_protocol_v0.1.json)、[no-TEST evaluator 任务](tasks/TASK_SLP_B09T_NO_TEST_EVALUATOR_RUNNER_v0.1.md)、[实现报告](stage_reports/S2_B09T_NO_TEST_EVALUATOR_RUNNER_v0.1.md)；PR #29 已独立复审 ACCEPT 并 squash merge `b671b8d`；no-TEST evaluator 固定 plurality/tie-break、unanimous reject、fixed metrics；CLI 仅 validate/synthetic，无 B01 TEST 或正式执行路径 | 独立审查 evaluator/runner；通过后另立正式 runner/authorization preparation；TEST 仍需一次性 Owner 精确授权 |
