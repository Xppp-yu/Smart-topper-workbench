...
最后更新：2026-08-29（B04A Implementation+Smoke 独立验收；未启动真实研究计算）
状态口径：只把已运行且存在可追溯产物的步骤标记为 `COMPLETE`；代码、计划或空目录不等于完成；验收前保持 `READY_FOR_REVIEW`。

## 当前一句话状态

PoPu P5.2-C 已接受 `small_resnet` 为固定睡姿五分类总体研究候选模型族；P6/P6.1 已完成 UNKNOWN/REJECT、错误复核、温度校准与三模型一致性模拟；P7 Full 软件扰动证据已完成并复核（14 conditions × 5 seeds × 15 folds），但不构成硬件、产品、整夜睡眠或控制安全验证。SLP A09R、B01–B04 已验收；B04 R05 在 TEST=0 下得到 SmallUNet `0.439625`、TinyFCN `0.051631`。由于原 Mini 只有一个正式分割候选与一个 Smoke 级控制候选，Owner 决定在 B07 前新增 B04A 受控架构扩展。B04A 三候选实现与 CPU 合成 Smoke 已通过 Codex 独立验收；runner integration、真实 GPU Mini、B07 Full 协议和任何 TEST 均未启动。
...
| S2_B04A | SLP8 PM-only 受控架构扩展 Mini | RUNNER_INTEGRATION_ACCEPTED / GPU_MINI_NOT_AUTHORIZED | [B04A Protocol](stage_reports/S2_B04A_SLP8_PM_ARCHITECTURE_EXPANSION_MINI_PROTOCOL_v0.1.md);[B04A Implementation+Smoke](stage_reports/S2_B04A_IMPLEMENTATION_SMOKE_v0.1.md);[B04A Runner Integration+Smoke](stage_reports/S2_B04A_RUNNER_INTEGRATION_SMOKE_v0.1.md);[B04A 交付说明](deliverables/SLP/B04A_PM_ARCHITECTURE_EXPANSION_MINI_PROTOCOL_v0.1.md);[B04A Implementation+Smoke 任务合同](tasks/TASK_SLP_B04A_IMPLEMENTATION_SMOKE_v0.1.md);[B04A Protocol 任务合同](tasks/TASK_SLP_B04A_PROTOCOL_FREEZE_v0.1.md);[B04A Runner Integration+Smoke 任务合同](tasks/TASK_SLP_B04A_RUNNER_INTEGRATION_SMOKE_v0.1.md);[配置](../configs/experiments/slp8_pm_architecture_expansion_mini_v0.1.json);3 候选: slp8_small_unet_v0.1 (incumbent, 118,121) / slp8_resunet_lite_v0.1 (120,809) / slp8_deeplabv3plus_lite_v0.1 (53,449); SegFormer-B0 DEFERRED; threshold 0.355644; seeds=[42,123,2026]; aug=none; param cap 300,000. tests/test_b04a_implementation.py 79/79 + tests/test_b04a_protocol_validator.py 50/50 + tests/test_slp8_region_models.py 38/38 + tests/test_slp8_region_mini.py 162/162 = **329/329 pytest passed**；protocol validator separately 30 OKs/0 errors. **RUNNER INTEGRATION ACCEPTED (2026-08-30)**: tests/test_b04a_runner_integration.py upgraded from R02 73/73 to **80/80** (R03 adds 7 cases: Reviewer reproduction advanced=("B","A") with 2nd pair=("A","C"); 3rd replaces first_loser; no 1st-near-tie then 2nd boundary compares top2 vs top3; both boundaries no near-tie; 2nd boundary fail-closed missing worst-subject; input order independence; advanced always two distinct candidates). _run_real_b01 keeps R02 protocol dispatch; _run_validate_config keeps R02 dual-branch split; B04AAdvanceTiebreak/B04AAdvanceDecision dataclass shapes unchanged but _b04a_advance_decision topology is FIXED by R03: 2nd boundary MUST compare first_loser (1st-round REJECTED) vs top3, NOT surviving top1/top2 vs top3; advanced = (first_winner, second_winner); R02 dedup fallback REMOVED; defensive fail-closed when first_winner == second_winner -> advanced=(). git diff --check clean; TEST=0; stage = RUNNER_INTEGRATION_ACCEPTED / GPU_MINI_NOT_AUTHORIZED | Codex Reviewer R03 independent audit passed; next Gate = publish this accepted change, then Owner may separately authorize TASK-SLP-B04A-MINI-RUN with an independent EXP-ID; GPU Mini remains NOT AUTHORIZED; MUST NOT claim GPU_MINI_AUTHORIZED / MINI_COMPLETE / B07_READY |
| S2_B07 | SLP8 PM-only Full 协议冻结 | BLOCKED_BY_B04A | 尚未开始；禁止把 B04A 文档完成当作模型或实验完成 | B04A 实际 Mini 通过 Reviewer 验收并冻结最多 1–2 个候选后才能开始 |
