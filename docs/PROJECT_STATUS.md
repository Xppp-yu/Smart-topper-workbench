...
| S2_B08 | SLP8 PM-only Full Runner 与 one-fold preflight runner | R01_FAILED_BEFORE_TRAINING / R02_RUNNER_FROZEN / GPU_REAUTH_PENDING | R01 clean SHA/environment/hash/validate-only 通过，但 real `RegionSample` 被误作 mapping 调 `.get()`，在首 epoch 前失败，且旧外层未写根级 FAILED；Round 7 已修复显式真实类型合同和异常 terminal 兜底，runner=`e0ba25a9aa0b33be971327ca398d822f7c7d1c8a`，B08 76 passed、py_compile/diff check 通过；TEST=0 | 推送 baseline 并制作 R02 bundle；R02 需 Owner 重新授权；30-unit Full 与 TEST 继续未授权 |
