...
| S2_B04A | SLP8 PM-only 受控架构扩展 Mini | PROTOCOL_ACCEPTED / IMPLEMENTATION_NOT_STARTED（R03） | [B04A Protocol](stage_reports/S2_B04A_SLP8_PM_ARCHITECTURE_EXPANSION_MINI_PROTOCOL_v0.1.md)；[B04A 交付说明](deliverables/SLP/B04A_PM_ARCHITECTURE_EXPANSION_MINI_PROTOCOL_v0.1.md)；[B04A 任务合同](tasks/TASK_SLP_B04A_PROTOCOL_FREEZE_v0.1.md)；[配置](../configs/experiments/slp8_pm_architecture_expansion_mini_v0.1.json)；3 候选：SmallUNet(incumbent,118,121)/ResUNet-lite(120,809,1x1 Conv2d shortcut)/DeepLabV3+-lite(53,449,Option A plain atrous)；SegFormer-B0 DEFERRED；阈值 0.355644；seeds=[42,123,2026]；aug=none；参数上限 300,000；Codex Reviewer 独立复核 56 tests、协议验证器、链接与 diff 通过；模型实现/Smoke NOT_RUN | 下一允许：另开 B04A Implementation+Smoke；GPU Mini 仍需 Owner 单独授权；B07 BLOCKED_BY_B04A |
