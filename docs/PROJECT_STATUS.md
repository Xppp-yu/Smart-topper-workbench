...
最后更新：2026-08-29（B04 真实 Mini R05 完成并通过本地证据验收）
状态口径：只把已运行且存在可追溯产物的步骤标记为 `COMPLETE`；代码、计划或空目录不等于完成；验收前保持 `READY_FOR_REVIEW`。

## 当前一句话状态

P5.2-C 已接受 `small_resnet` 为 PoPu 固定睡姿五分类总体研究候选模型族。P6/P6.1 已完成 UNKNOWN/REJECT、原始重点案例复核、温度校准与三模型一致性模拟；P7 真实 checkpoint 扰动推理仍待运行。SLP A09R 已以 `COMPLETE_WITH_LIMITATIONS` 验收：`SLP_8Region_Pressure_VAL_v1.1`（4,590 samples，102 danaLab，8 区）确立为当前项目 SLP8 pressure-only 区域分割参考 GT。SLP B01、B02、B03 已验收。B04 真实 Mini R05 已在 RTX 4090 上完成：SmallUNet 的 VAL fixed foreground macro IoU 为 `0.439625`，通过冻结门槛 `0.205644`；TinyFCN 为 `0.051631`，未通过。全程 TEST 读取数为 0，归档 SHA-256 已在 Windows 工作台独立核验；下一步是 B07 Full 协议冻结，不直接运行 Full。
...
| S1 | 跨模态配对、homography 与 overlay 审计 | IN_PROGRESS_WITH_A05_A06_A07_A08_DELIVERED | [A03 Frame Master Index](stage_reports/S1_1_SLP_FRAME_MASTER_INDEX_v0.1.md)、[A04 Homography 审计](stage_reports/S1_2_SLP_HOMOGRAPHY_AUDIT_v0.1.md)、[A05 Canonical Adapter](stage_reports/S1_3_SLP_CANONICAL_ADAPTER_v0.1.md)、[A06 Subject Split Freeze](stage_reports/S1_4_SLP_SUBJECT_SPLIT_FREEZE_v0.1.md)、[A07 Joint Occlusion EDA](stage_reports/S1_5_SLP_JOINT_EDA_v0.1.md)、[A08 Body Axis Geometry](stage_reports/S1_8_SLP_BODY_AXIS_GEOMETRY_v0.1.md)、[A09R GT Contract Realign](stage_reports/S1_A09R_SLP8_GT_CONTRACT_REALIGN_v0.1.md)、[Examples](stage_reports/SLP_CANONICAL_SAMPLE_EXAMPLES_v0.1.md)、[SLP 两阶段总计划](SLP_TWO_PHASE_CONTINUOUS_DEVELOPMENT_PLAN_v0.2.md)、[Agent 任务清单](SLP_AGENT_TASK_BACKLOG_v0.1.md) | A09R 完成：SLP_8Region_Pressure_VAL_v1.1 已接受为项目参考 GT（4,590 samples，102 danaLab，8 区）；A10–A17 路线改为 HOLD/SUPERSEDED；B01–B04 已完成，B07 协议冻结可开始 |
...
| S2_B04 | SLP8 PM-only Region Mini | DONE_WITH_LIMITATIONS | [S2_B04 报告](stage_reports/S2_B04_SLP8_PM_ONLY_REGION_MINI_PROTOCOL_v0.1.md)；真实 R05：TRAIN 3,645 / VAL 450 / TEST 0，RTX 4090，状态 `DONE`，归档 SHA-256=`57885db2...87c19`；SmallUNet VAL fixed foreground macro IoU `0.439625`（`FEASIBLE`），TinyFCN `0.051631`（`NOT_FEASIBLE`）；两候选 reload hash 一致；4 个真实 GPU 运行缺陷已由 `f3fb7d9`、`c4ebc5d`、`762f44e`、`72fbe67` 修复 | B07 Full 协议冻结 `READY`；只冻结协议与候选，不在 B07 运行 Full；保留 NOT_REVIEWED、uncover-only、无 TEST 和非产品结论边界 |
