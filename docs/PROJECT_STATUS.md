...
| S2_B09T | SLP8 一次性最终 TEST 评价 | PROTOCOL_READY_FOR_REVIEW / TEST_DENIED / EXECUTION_NOT_AUTHORIZED | [协议任务](tasks/TASK_SLP_B09T_FINAL_TEST_PROTOCOL_FREEZE_v0.1.md)、[协议报告](stage_reports/S2_B09T_FINAL_TEST_PROTOCOL_FREEZE_v0.1.md)、[冻结配置](../configs/experiments/slp8_pm_b09t_final_test_protocol_v0.1.json)；绑定 B11F 三 final SHA；primary=三 seed hard majority；primary metric=pooled fixed foreground macro IoU；11 subjects / 495 samples 仅为结构合同，未读取 | 独立审查协议；通过后实现 no-TEST evaluator/runner；正式 TEST 仍需一次性 Owner 精确授权 |
