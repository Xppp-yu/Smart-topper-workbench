...
| J1_PM | Homography 派生（PM 空间） | **84×192**（修正） | align_PTr_RGB.npy 变换 J0 |
...
> **PM 图像尺寸修正（v0.2）**：PM PNG 实际尺寸为宽 84 × 高 192。
> 初始版本误写为 192×84，已在修复提交中修正。以下数据基于正确的 84×192 边界。

| 节点 | 遮挡率 | 越界数 | 越界率 | J1 x 范围 | J1 y 范围 |
|------|--------|--------|--------|-----------|-----------|
| head_cervical | 85.0% | 75 | 0.51% | [-4.5, 85.3] | [76.2, 196.9] |
| neck_c7 | 84.8% | 162 | 1.11% | [-6.7, 93.0] | [40.7, 159.3] |
| right_shoulder | 69.6% | 0 | 0.00% | [8.3, 73.9] | [68.9, 130.0] |
| right_elbow | 69.2% | 3 | 0.02% | [13.5, 84.9] | [68.0, 126.5] |
| right_wrist | 84.9% | 122 | 0.83% | [-6.5, 91.3] | [40.8, 159.8] |
| left_shoulder | 83.3% | 93 | 0.64% | [-4.9, 93.0] | [70.2, 198.3] |
| left_elbow | 76.9% | 169 | 1.16% | [-15.2, 88.1] | [-3.0, 138.8] |
| left_wrist | 89.7% | 236 | 1.61% | [-10.7, 86.0] | [-4.9, 116.6] |
| right_hip | 74.4% | 9 | 0.06% | [-7.2, 78.1] | [15.1, 84.3] |
| right_knee | 74.1% | 3 | 0.02% | [1.9, 84.5] | [13.4, 82.6] |
| right_ankle | 91.0% | 209 | 1.43% | [-6.9, 96.6] | [-8.5, 108.4] |
| left_hip | 78.0% | 113 | 0.77% | [-8.8, 94.4] | [-2.2, 130.0] |
| left_knee | 92.6% | 0 | 0.00% | [3.0, 73.0] | [11.6, 73.8] |
| left_ankle | 99.2% | 343 | 2.35% | [-16.7, 87.8] | [-8.7, 66.7] |
| **总计** | — | **1,537** | **0.75%** | — | — |

**关键发现：**
- J1 遮挡率与 J0 完全一致（Homography 不改变置信度）
- J1 越界率（0.75%）显著低于 J0（1.35%），因为 Homography 将节点拉回 PM 有效区域
- left_ankle 越界数最多（343，2.35%），与 J0 一致（俯视导致足部超出图像边界）
- J1 y 范围扩展到 [-8.7, 198.3]，超出 PM 高度 192 的节点分布在上下边界
...
### 8.3 J1 越界节点分布（PM 总越界数：1,537）

| 节点 | 越界数 | 占比 |
|------|--------|------|
| left_ankle | 343 | 22.3% |
| left_wrist | 236 | 15.4% |
| right_ankle | 209 | 13.6% |
| left_elbow | 169 | 11.0% |
| neck_c7 | 162 | 10.5% |
| right_wrist | 122 | 7.9% |
| left_shoulder | 93 | 6.1% |
| left_hip | 113 | 7.4% |
| 其他 | 90 | 5.8% |

**发现**：J1 越界分布与 J0 不同，腕部和足部越界占比更高，因为 Homography 变换将坐标映射到 PM 空间时，部分节点仍然超出 PM 边界。
...
3. **越界问题集中于头部（J0）和足部（J1）**：J0 中头部节点占越界总数 70%；J1 中足部越界占比最高（left_ankle 22.3%）。
4. **J1 PM 越界率低于 J0**：J1 越界率 0.75% vs J0 越界率 1.35%，Homography 变换将大部分节点拉回 PM 有效区域。
5. **PM 图像尺寸已修正**：PM PNG 实际尺寸为宽 84 × 高 192（初始版本误写为 192×84，已修正）。
6. **骨段长度样本分布不均衡**：足部相关骨段的有效样本数远少于上肢骨段，与遮挡率分布一致。
7. **Quarantine 样本较少**：仅 90 个（0.6%），不影响整体统计。
8. **Train/Val/Test 分布合理**：72.3% / 12.6% / 15.1%，符合标准划分。
...
5. **J1 坐标系适合 PM 对齐任务**：J1 坐标在 PM 空间内（84×192），适合与 PM 图像对齐的任务（如 SMPL-X 拟合）。
6. **J1 PM 越界节点需要裁剪**：J1 中 left_ankle 越界率最高（2.35%），建议在 J1 预处理时进行 PM 边界裁剪。
...
- ✅ 不训练区域模型
- ✅ 异常帧已记录供后续分析
- ✅ PM 图像尺寸修正（84×192，已重新运行 J1 越界统计）
- ✅ A04 Homography 代码未被修改
