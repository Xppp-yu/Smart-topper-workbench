...
from topper_perception.neural.slp8_region_final_fit import FinalFitError, load_protocol, run_final_fit
...
def main() -> int:
    ...
    try:
        ...
        if not all((args.experiment_id, args.output_dir, args.b01_freeze_dir, args.dataset_root)): raise FinalFitError("real final fit requires experiment/output/freeze/dataset arguments")
        sha, dirty = git_identity()
        run_final_fit(protocol=protocol, freeze_dir=args.b01_freeze_dir, data_root=args.dataset_root, output_dir=args.output_dir, experiment_id=args.experiment_id, git_commit=sha, git_dirty=dirty, resume=args.resume_authorized)
        return 0
