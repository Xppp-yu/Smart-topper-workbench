...
截至 2026-08-19：P0–P5.2-A 已完成。P5.2-A 的 CPU 与 RTX 4090 CUDA Smoke 均已通过，训练、checkpoint/resume/reload 和固定 seed 最小复现链路成立；Smoke 指标不用于模型排名。P5.1 的 `calibrated_linear_svm` 继续保留为**传统模型候选**（record macro-F1 0.9452），不是已经覆盖 CNN 的总体最优模型。P5.2-B Mini 筛选的协议、代码、版本化配置与测试已冻结并达到 `MINI_READY_TO_RUN`（`runner_type=popu_neural_mini`，冻结配置 `configs/experiments/popu_neural_mini_v0.1.json`，协议报告 `docs/stage_reports/P5_2_B_POPU_NEURAL_MINI_PROTOCOL_v0.1.md`；本任务**未运行**真实 Mini/Full）。下一交接点为需 Reviewer 只读复核、配置冻结并 Controller 授权后，才由 Experiment Runner 在 AutoDL 执行 P5.2-B Mini Run。P5.2-C 完成总体候选选择并经 Reviewer 接受后才放行 P6 `UNKNOWN/REJECT`。P4b 人体区域监督因 PoPu 标注无法唯一配对而继续 HOLD。
